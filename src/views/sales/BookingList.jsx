...
const BookingList = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const pendingBookings = branchBookings.filter(booking => 
      ...
      const approvedBookings = branchBookings.filter(booking => 
        booking.status === 'APPROVED'
      );
      setApprovedData(approvedBookings);
      setFilteredApproved(approvedBookings);
      
      const allocatedBookings = branchBookings.filter(booking => 
        booking.status === 'ALLOCATED'
      );
          ...
          <table className="responsive-table" style={{ overflow: 'auto' }}>
            <thead className="table-header-fixed">
              <tr>
                ...
                <th>Booking Date</th>
                {/* <th>Upload Finance</th> */}
                {tabIndex != 2 &&  <th>Upload Finance</th>}
                <th>Upload KYC</th>
                <th>Status</th>
                {tabIndex === 2 && <th>Chassis Number</th>}
                {tabIndex === 2 && <th>Is Claim</th>}
                <th>Print</th>
                <th>Action</th>
              </tr>
            </thead>
      ...
      {booking.claimDetails?.hasClaim ? (
        ...
        <FaTimesCircle className="status-icon inactive-icon" />
                    ...
                    <td>
                      ...
                      <Menu id={`action-menu-${booking.id}`} anchorEl={anchorEl} open={menuId === booking.id} onClose={handleClose}>
                        <MenuItem onClick={() => handleViewBooking(booking.id)}>View Booking</MenuItem>
                        {tabIndex != 2 && <Link className="Link" to={`/booking-form/${booking.id}`}>
                          <MenuItem>Edit</MenuItem>
                        </Link>
                        }
                        {booking.payment.type === 'FINANCE' && booking.documentStatus?.financeLetter?.status !== 'NOT_UPLOADED' && (
