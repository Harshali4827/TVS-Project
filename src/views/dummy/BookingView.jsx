...
const ViewBooking = ({ open, onClose, booking, refreshData }) => {
  ...
  const handleUpdateBookingStatus = async (action, note, chassisNumber) => {
    try {
      ...
      if (action === 'approve' && chassisNumber) {
        requestBody.chassisNumber = chassisNumber;
      }

      await axiosInstance.post(`/bookings/${booking._id}/${action}`, requestBody);
  ...
  const handleActionWithNote = async (action) => {
    if (action === 'approve') {
      const { value: formValues } = await Swal.fire({
        title: 'Approve Booking',
        html:
          '<input id="swal-input1" class="swal2-input" placeholder="Approval Note">' +
          '<input id="swal-input2" class="swal2-input" placeholder="Chassis Number">',
        focusConfirm: false,
        showCancelButton: true,
        confirmButtonText: 'Submit',
        cancelButtonText: 'Cancel',
        ...
        inputValidator: (values) => {
          if (!values[0]) return 'Approval note is required!';
          if (!values[1]) return 'Chassis number is required!';
        }
      });

      if (formValues) {
        await handleUpdateBookingStatus(action, formValues[0], formValues[1]);
      }
    } else {
      // Original handling for other actions
      const { value: note } = await Swal.fire({
        ...
        cancelButtonText: 'Cancel',
        inputValidator: (value) => {
          if (!value) return 'Approval note is required!';
        }
      });

      if (note) {
        await handleUpdateBookingStatus(action, note);
      }
    }
  };
  ...
  //     return <CBadge color="secondary">Not Uploaded</CBadge>;
  //   }
  ...
  //   return <CBadge color={badgeColor}>{doc.status}</CBadge>;
  // };

  const renderDocumentStatus = (doc, type) => {
    if (!doc || doc.status === 'NOT_UPLOADED' || doc.status === 'REJECTED') {
      return (
  ...
  const renderExchangeDetails = () => {
    if (!booking.exchange) return <span>No</span>;
  ...
  const renderFinanceDetails = () => {
    if (booking.payment.type !== 'FINANCE') return <span>N/A</span>;
                ...
                <div className="details-column">
                  ...
                  <CCardBody>
                    <div className="detail-row">
                      ...
                      <span className="detail-value">
                        {booking.documentStatus?.kyc ? (
                          renderDocumentStatus(booking.documentStatus.kyc)
                        ) : (
                          <CBadge color="secondary">Not Uploaded</CBadge>
                        )}
                      </span>
                    </div>
                    {booking.payment.type === 'FINANCE' && (
                      <div className="detail-row">
                        ...
                        <span className="detail-value">
                          {booking.documentStatus?.financeLetter ? (
                            renderDocumentStatus(booking.documentStatus.financeLetter)
                          ) : (
                            <CBadge color="secondary">Not Uploaded</CBadge>
                          )}
                        </span>
                  ...
                  <CCard className="booking-section">
                    ...
                    <CCardBody>
                      <div className="detail-row">
                        ...
                        <span className="detail-value">
                          {booking.documentStatus?.kyc ? (
                            renderDocumentStatus(booking.documentStatus.kyc, 'kyc')
                          ) : (
                            <div className="d-flex align-items-center">
                              <CBadge color="secondary" className="me-2">
                                Not Uploaded
                      ...
                      {booking.payment.type === 'FINANCE' && (
                        <div className="detail-row">
                          ...
                          <span className="detail-value">
                            {booking.documentStatus?.financeLetter ? (
                              renderDocumentStatus(booking.documentStatus.financeLetter, 'finance')
                            ) : (
                              <div className="d-flex align-items-center">
                                <CBadge color="secondary" className="me-2">
                                  Not Uploaded
          ...
          <div className="d-flex justify-content-between w-100">
            <div>
              {booking?.status !== 'APPROVED' && (
                <button className="btn btn-success me-2" onClick={() => handleActionWithNote('approve')} disabled={actionLoading}>
                  {actionLoading ? 'Approving...' : 'Approve'}
                </button>
              )}
              {booking?.status !== 'REJECTED' && (
                <button className="btn btn-danger me-2" onClick={() => handleActionWithNote('reject')} disabled={actionLoading}>
                  {actionLoading ? 'Rejecting...' : 'Reject'}
                </button>
              )}
              {booking?.status !== 'COMPLETED' && (
                <button className="btn btn-primary me-2" onClick={() => handleActionWithNote('complete')} disabled={actionLoading}>
                  {actionLoading ? 'Completing...' : 'Complete'}
                </button>
              )}
              {booking?.status !== 'CANCELLED' && (
                <button className="btn btn-warning me-2" onClick={() => handleActionWithNote('cancel')} disabled={actionLoading}>
