...
//                 </CFormSelect>
//                 {!selectedSubdealer && (
//                   <div className="form-text text-warning">Please select a subdealer to continue</div>
//                 )}
//               </div>
...
//                     </div>
//                     {!selectedMonth && (
//                       <div className="form-text text-warning mt-2">Please select a month</div>
//                     )}
//                   </div>
//                 </CCol>
//               </CRow>
...
//                     Please complete all required fields to generate a report
//                   </div>
//                 )}
//               </div>
//             </CCardBody>
...
//         size="lg"
//         scrollable
//         className="commission-report-modal"
//       >
//         <CModalHeader>
//           <CModalTitle>
...
//                                 <p><strong>Commission:</strong> {formatCurrency(booking.total_commission)}</p>
//                               </CCol>
//                             </CRow>
//                           </div>
...
//                                   <tr key={idx}>
//                                     <td>{item.header}</td>
//                                     <td className="text-end">{formatCurrency(item.base)}</td>
...
//                               <tfoot>
//                                 <tr>
//                                   <th colSpan={3} className="text-end">Total Commission:</th>
...
//             Close
//           </CButton>
//           <CButton color="primary" onClick={handlePrint}>Print Report</CButton>
//         </CModalFooter>
//       </CModal>
...
const CalculateCommission = () => {
  ...
  const generateMonthOptions = () => {
    ...
    // Auto-select current month if it's available and no month is selected
    if (!selectedMonth && parseInt(selectedYear) === currentYear) {
      setSelectedMonth(currentMonth);
    } else if (!selectedMonth) {
      setSelectedMonth(1); // Default to January for past years
    }
  };
  ...
  const handleSubdealerChange = (e) => {
    setSelectedSubdealer(e.target.value);
    setReportData(null); // Clear previous report when subdealer changes
  };

  const handleYearChange = (e) => {
    setSelectedYear(parseInt(e.target.value));
    setSelectedMonth(''); // Reset month when year changes
    setReportData(null); // Clear previous report when year changes
  };
...
const handlePrintCommissionLedger = () => {
  ...
  const subdealerName = subdealers.find(s => s._id === selectedSubdealer)?.name || 
    ...
    <CContainer className="my-4">
      ...
      >
        ...
        <CModalFooter>
          ...
          <CButton color="primary" onClick={handlePrintCommissionLedger}>Print Ledger</CButton>
        </CModalFooter>
      </CModal>
