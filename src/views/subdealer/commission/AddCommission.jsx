...
import '../../../css/form.css';
import { CInputGroup, CInputGroupText, CFormSelect, CFormInput, CTable, CTableHead, CTableRow, CTableHeaderCell, CTableBody, CTableDataCell } from '@coreui/react';
import CIcon from '@coreui/icons-react';
import { cilUser, cilBuilding, cilLocationPin } from '@coreui/icons';
...
function AddCommission() {
  ...
  const handleCommissionRateChange = (headerId, value) => {
    setFormData(prev => {
      const updatedRates = prev.commission_rates.map(rate => 
  ...
  if (formData.commission_rates.length === 0) {
    ...
    const invalidRates = formData.commission_rates.filter(rate => 
                  ...
                  <CTable striped>
                    <CTableHead>
                      <CTableRow>
                        ...
                        <CTableHeaderCell>Price Value</CTableHeaderCell>
                        <CTableHeaderCell>Commission Rate (%)</CTableHeaderCell>
                      </CTableRow>
                    </CTableHead>
                    <CTableBody>
                      {selectedModel.prices.map((price, index) => {
                        const commissionRate = formData.commission_rates.find(
                          rate => rate.header_id === price.header._id
                        );
                        
                        return (
                          <CTableRow key={price.header._id}>
                            <CTableDataCell>{price.header.header_key}</CTableDataCell>
                            <CTableDataCell>{price.value}</CTableDataCell>
                            <CTableDataCell>
                              <CFormInput
                                type="number"
