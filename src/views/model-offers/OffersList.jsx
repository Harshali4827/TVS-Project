...
const OffersList = () => {
  ...
  const handlePdfExport = () => exportToPdf(data, ['title', 'description', 'isActive', 'applyToAllModels', 'applicableModels'], 'Offers');
      ...
      <div className="table-container">
        <div className="table-header">
          <div className="search-icon-data">
            <input type="text" placeholder="Search.." onChange={(e) => handleFilter(e.target.value, getDefaultSearchFields('offers'))} />
            <SearchOutlinedIcon />
          </div>
                ...
                {currentRecords.length === 0 ? (
                  ...
                  currentRecords.map((offer, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <button className="action-button" onClick={(event) => handleClick(event, offer._id)}>
                          Action
                        </button>
                        <Menu id={`action-menu-${offer._id}`} anchorEl={anchorEl} open={menuId === offer._id} onClose={handleClose}>
